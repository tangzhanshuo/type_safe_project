...
object Routes:
  private def executePlan(messageType: String, str: String): IO[String] =
    messageType match {
      ...
      case "AdminGetAllCoursesMessage" =>
        IO(decode[AdminGetAllCoursesMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for AdminGetAllCoursesMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case "AdminGetCourseByCourseIDMessage" =>
        IO(decode[AdminGetCourseByCourseIDMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for AdminGetCourseByCourseIDMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case "AdminGetCourseByCourseNameMessage" =>
        IO(decode[AdminGetCourseByCourseNameMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for AdminGetCourseByCourseNameMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case "AdminGetCourseByTeacherUsernameMessage" =>
        IO(decode[AdminGetCourseByTeacherUsernameMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for AdminGetCourseByTeacherUsernameMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      ...
      case "AdminGetPlanListMessage" =>
        IO(decode[AdminGetPlanListMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for GetPlanListMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case "AdminUpdateCoursePriorityMessage" =>
        IO(decode[AdminUpdateCoursePriorityMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for AdminUpdateCoursePriorityMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case "AdminAddPlanMessage" =>
        IO(decode[AdminAddPlanMessagePlanner](str).getOrElse(throw new Exception("Invalid JSON for GetPlanListMessage")))
          .flatMap { m =>
            m.fullPlan.map(_.asJson.noSpaces)
          }
      case _ =>
