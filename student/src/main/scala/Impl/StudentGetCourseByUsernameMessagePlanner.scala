...
case class StudentGetCourseByUsernameMessagePlanner(studentUsername: String, override val planContext: PlanContext) extends Planner[Option[List[StudentCourse]]]:
  override def plan(using PlanContext): IO[Option[List[StudentCourse]]] = {
    getCourseByStudentUsername(studentUsername).map {
      case Some(courses) =>
